...
import End from './components/Endgame'
...
class MatchGame extends Component {
  ...
  tick = () => {
    ...
    if (time === 0) {
      clearInterval(this.timer)
      this.setState({time: 0, toggle: true})
    }
  }
  ...
  clicked1 = id => {
    ...
    const filt = imagesList.filter(each => each.id === imagesList[thumb].id)
  ...
  render() {
    ...
    return (
      <div className="mainbg">
        <Nav tim={time} count={count} />
        {toggle ? (
          <End score={count} click={this.bak} />
        ) : (
          <div className="sec">
            <ul>
